...
export class CreatebooksComponent {
  ...
  onslected(event: any): void {
    if (event.target.files) {
      ...
      if (!allocatefile.includes(file.type)) {
        this.snackBar.open('only JPG PNG JPEG file are allowed', 'Close', {
          duration: 5000,
        });
        this.bookform.patchValue({
          imageUrl: null,
      ...
      if (file.size > filesize) {
        this.snackBar.open('file size should not exceed than 2MB', 'Close', {
          duration: 5000,
        });
        this.bookform.patchValue({
          imageUrl: null,
  ...
  loadbooks(id: number): void {
    ...
    this.booksser.getbookbyID(id).subscribe(
      (book: Book) => {
        this.bookform.patchValue({
          Title: book.Title,
          Author: book.Author,
  ...
  addOrUpdateBook(): void {
    if (this.bookform.valid) {
      ...
      if (this.isEditing && this.currentBookId) {
        this.booksser
          .updatebook({ ...newbook, id: this.currentBookId })
          .pipe(switchMap(() => this.booksser.getbooks())
          ,catchError((error)=>{
